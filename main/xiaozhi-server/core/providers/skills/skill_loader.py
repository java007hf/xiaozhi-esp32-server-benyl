...
class SkillLoader:
    ...
    def build_catalog_prompt_block(self) -> str:
        ...
        lines = [
            ...
            "You have file-based skills available. Enabled skills are supported capabilities, even when the domain action is not exposed as a direct function name.",
            "The catalog below is only a routing index, not the full instructions.",
            "Before giving a final answer or using `direct_answer`, apply this routing policy:",
            "- If the user's request probably matches any listed skill, call `activate_skill` with that exact skill name first.",
            "- Do not say you cannot perform a task that matches an enabled skill until you have activated the skill and tried the documented tool path.",
            "- If ASR or user text contains likely homophones, typos, or aliases, route by the intended domain when context clearly matches a skill.",
            "- If the task spans multiple domains, activate the primary skill first, then activate additional relevant skills as needed.",
            "- Ask a clarification only when required information is actually missing and cannot be inferred from context or configuration.",
